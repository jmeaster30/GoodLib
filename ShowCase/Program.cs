...
//Console.WriteLine($"Bytes {bytes.ToHexString()}");
//Console.WriteLine($"Encoded {encoded.ToHexString()}");
//Console.WriteLine($"Decoded {decoded.ToHexString()}");
Console.WriteLine($"Size: {bytes.Count()} Encoded Size: {encoded.Count()} Ratio {encoded.Count() / (float)bytes.Count()}");
